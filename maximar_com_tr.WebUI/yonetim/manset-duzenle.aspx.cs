        ...
        protected void Kayit()
        {
            if (Request.QueryString["ID"] != null && Class.Fonksiyonlar.NumerikKontrol(Request.QueryString["ID"].ToString()))
            {
                ...
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    ...
                    if (SQL.Any())
                    {
                        lit_onizleme.Text = "<img style=\"border:1px dotted #CCC;\" src=\"../ashx/image-view.ashx?i=upload/header/" + Class.Yonetim.Fonksiyonlar.Manset(ID).Select(p => p.dosya_ad).FirstOrDefault() + "&amp;w=300&amp;h=150&amp;k=t\" />";
                        ...
                        foreach (var i in SQL)
                        {
                            ...
                            baslik.Text = Class.Yonetim.Fonksiyonlar.MansetDetay(ID, i.id).Select(p => p.detay).FirstOrDefault();
        ...
        protected void KayitEkle(int manset_id, int dil_id)
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ...
                if (SQL.Any())
                {
                    ...
                    tbl.detay = Request.Form["txt_baslik_" + dil_id].ToString().Trim();
                ...
                if (Request.Files["FileUpload1"].FileName.ToString() != "")
                {
                    ...
                    postedFile.SaveAs(Server.MapPath("/upload/header/" + ResimAdi + ""));
                    tbl2.dosya_ad = ResimAdi;
                }

                tbl2.admin_id_gun = int.Parse(HttpContext.Current.Request.Cookies[Class.Fonksiyonlar.Ayar(Class.Sabitler.Ayar.GuvenlikKodu) + "KullaniciID"].Value);
