    ...
    public partial class video_galeri_duzenle : System.Web.UI.Page
    {
        ...
        protected void Kayit()
        {
            if (Request.QueryString["ID"] != null && Class.Fonksiyonlar.NumerikKontrol(Request.QueryString["ID"].ToString()))
            {
                ...
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    ...
                    if (SQL.Any())
                    {
                        lit_onizleme.Text = "<img style=\"border:1px dotted #CCC;\" src=\"../ashx/image-view.ashx?i=upload/video-gallery/" + Class.Yonetim.Fonksiyonlar.VideoGaleri(ID).Select(p => p.dosya_ad).FirstOrDefault() + "&amp;w=300&amp;h=150&amp;k=t\" />";
        ...
        protected void KayitEkle(int haber_id, int dil_id)
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ...
                tbl2.baslik = Request.Form["txt_tanimlama_baslik"].ToString().Trim();
