        ...
        protected void Kayit()
        {
            if (Request.QueryString["ID"] != null && Class.Fonksiyonlar.NumerikKontrol(Request.QueryString["ID"].ToString()))
            {
                ...
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    ...
                    if (SQL.Any())
                    {
                        lit_onizleme.Text = "<img style=\"border:1px dotted #CCC;\" src=\"../ashx/image-view.ashx?i=upload/news/" + Class.Yonetim.Fonksiyonlar.Haber(ID).Select(p => p.dosya_ad).FirstOrDefault() + "&amp;w=300&amp;h=150&amp;k=t\" />";
        ...
        protected void KayitEkle(int haber_id, int dil_id)
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ...
                if (Request.Files["FileUpload1"].FileName.ToString() != "")
                {
                    ...
                    postedFile.SaveAs(Server.MapPath("/upload/news/" + ResimAdi + ""));
